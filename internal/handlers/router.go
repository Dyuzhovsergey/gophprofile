...
func NewRouter(
	...
	// Технические endpoints не ограничиваются Rate Limiting.
	...
	// Ограничиваем только пользовательские динамические маршруты.
	router.Group(func(r chi.Router) {
		...
		r.Route("/api/v1", func(r chi.Router) {
			r.Get(
				"/avatars/{avatar_id}",
				avatarHandler.GetByID,
			)
			r.Get(
				"/avatars/{avatar_id}/metadata",
				avatarHandler.GetMetadata,
			)

			r.Get(
				"/users/{user_id}/avatar",
				avatarHandler.GetCurrentByUserID,
			)
			r.Get(
				"/users/{user_id}/avatars",
				avatarHandler.ListByUserID,
			)
			...
			r.Group(func(r chi.Router) {
				...
				r.Delete(
					"/avatars/{avatar_id}",
					avatarHandler.DeleteByID,
				)
				r.Delete(
					"/users/{user_id}/avatar",
					avatarHandler.DeleteCurrentByUserID,
				)
			})
		})
	})
